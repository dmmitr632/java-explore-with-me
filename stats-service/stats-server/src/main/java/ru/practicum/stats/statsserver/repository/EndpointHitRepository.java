package ru.practicum.stats.statsserver.repository;
...
public interface EndpointHitRepository extends JpaRepository<EndpointHit, Integer> {
    ...
    @Query("SELECT new ru.practicum.stats.statsdto.dto.ViewStatsDto(eh.app, eh.uri, count(distinct eh.ip)) " +
            "FROM EndpointHit eh " +
    ...
    @Query("SELECT new ru.practicum.stats.statsdto.dto.ViewStatsDto(eh.app, eh.uri, count(eh)) " +
            "FROM EndpointHit eh " +
            "WHERE eh.timestamp >= ?1 AND eh.timestamp <= ?2 AND eh.uri in ?3 " +
